...
function head({ title, description, canonical, image = `${siteUrl}/assets/reimage-logo-2026-transparent.png`, schema = [] }) {
  ...
  <link rel="stylesheet" href="/marketplace.css?v=20260904-2">
...
function card(business, options = {}) {
  ...
  return `<article class="business-card${featured ? ' business-card--featured' : ''}" data-business-card data-slug="${business.slug}" data-primary-category="${business.categories[0]}" data-categories="${business.categories.join(' ')}" data-location="${(business.locations || [business.locationKey]).join(' ')}" data-rank="${business.regionRank}" data-search="${escapeHtml(searchText)}">
    <a class="business-card__media" href="${profileUrl(business.slug)}" aria-label="View ${escapeHtml(business.name)} profile">
      <img src="${business.image.startsWith('http') ? business.image : `/${business.image}`}" alt="${escapeHtml(business.imageAlt)}" width="960" height="600" loading="lazy" decoding="async">
  ...
  const body = `<main id="main-content">
    <section class="market-hero">
      <div class="market-wrap market-hero__inner">
        <p class="eyebrow"><span></span> Hartford Marketplace by RE IMAGE</p>
        <h1>Find Hartford’s best<br><em>local businesses.</em></h1>
        ...
        <div class="quick-searches" aria-label="Popular searches"><span>Popular:</span><button type="button" data-quick-search="peanut punch">Peanut Punch</button><button type="button" data-quick-search="online estimate">Online estimates</button><button type="button" data-quick-search="braids">Braids</button><button type="button" data-quick-search="quesabirria">Quesabirria</button></div>
        <div class="market-trust"><span><strong>${hartfordBusinesses.length}</strong> Hartford-area businesses</span><span><strong>${data.categories.length}</strong> service categories</span><span><strong>100%</strong> direct business links</span></div>
      </div>
    </section>
